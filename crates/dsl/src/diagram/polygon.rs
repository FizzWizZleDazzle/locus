...
pub fn render(spec: &Polygon, vars: &VarMap) -> Result<String, DslError> {
    ...
    if n < 3 {
        return Err(DslError::Evaluation(
            "polygon requires at least 3 vertices".into(),
        ));
    }
    ...
    let explicit_coords: Option<Vec<(f64, f64)>> = spec
        .vertices
        .iter()
        .map(|v| match v {
            Vertex::Coord { x, y, .. } => {
    ...
    let pts_raw: Vec<(f64, f64)> = if let Some(p) = explicit_coords {
        p
    } else {
...
fn compute_layout(
    ...
    for i in 0..(n - 1) {
        let (cx, cy) = pts[i];
        pts.push((
            cx + lengths[i] * heading.cos(),
            cy + lengths[i] * heading.sin(),
        ));
