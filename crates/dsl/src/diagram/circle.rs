...
pub fn render(spec: &Circle, _vars: &VarMap) -> Result<String, DslError> {
    ...
    for el in &spec.elements {
        match el {
            CircleElement::Chord(c) => {
                push(&mut named_pts, &c.from);
                push(&mut named_pts, &c.to);
            }
            CircleElement::Arc(a) => {
                push(&mut named_pts, &a.from);
                push(&mut named_pts, &a.to);
            }
            CircleElement::Radius(rd) => {
                push(&mut named_pts, &rd.to);
            }
            CircleElement::Tangent(t) => {
                push(&mut named_pts, &t.at);
            }
            ...
            CircleElement::Point(p) => {
                ...
                explicit.insert(
                    p.name.clone(),
                    eval_num(&p.angle, _vars).unwrap_or(0.0).to_radians(),
                );
            }
        }
    }
    ...
    let n_auto: usize = on_circle
        .iter()
        .filter(|n| !explicit.contains_key(n.as_str()))
        .count();
    let auto_step = if n_auto > 0 {
        TAU / (n_auto as f64)
    } else {
        0.0
    };
    ...
    for el in &spec.elements {
        match el {
            CircleElement::Chord(c) => {
                let p = positions[&c.from];
                let q = positions[&c.to];
            ...
            CircleElement::Arc(a) => {
                ...
                if let Some(label) = &a.label {
                    ...
                    cetz::content(
                        &mut s,
                        (r * 0.7 * mid.cos(), r * 0.7 * mid.sin()),
                        &format_label(label, _vars, ""),
                    );
                }
            }
            CircleElement::Radius(rd) => {
