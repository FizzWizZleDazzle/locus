...
pub fn render(spec: &ForceDiagram, vars: &VarMap) -> Result<String, DslError> {
    let incline_deg = spec.incline_angle.as_ref()
    ...
    match spec.surface {
        SurfaceKind::Flat => {
            cetz::line(&mut s, (-3.0, -0.5), (3.0, -0.5), Color::Black, LineStyle::Solid);
        }
        SurfaceKind::Incline => {
            let l = 4.0;
            let cos = incline.cos(); let sin = incline.sin();
            let p0 = (-l/2.0 * cos, -l/2.0 * sin);
            let p1 = (l/2.0 * cos, l/2.0 * sin);
            cetz::line(&mut s, p0, p1, Color::Black, LineStyle::Solid);
            cetz::line(&mut s, (p0.0 - 0.4, p0.1), (p1.0, p0.1), Color::Black, LineStyle::Solid);
        }
        SurfaceKind::None => {}
    }
    ...
    match spec.object {
        ObjectKind::Block | ObjectKind::Beam => {
            let (w, h) = if matches!(spec.object, ObjectKind::Beam) { (0.8, 0.2) } else { (0.5, 0.5) };
            let pts = [
    ...
    for force in &spec.forces {
        let (theta, label, color) = match force {
            ForceEntry::Gravity(b) => (
                -PI / 2.0, b.label.clone().unwrap_or_else(|| "mg".into()), Color::Red,
            ),
            ForceEntry::Normal(b) => {
                let t = if matches!(spec.surface, SurfaceKind::Incline) { PI/2.0 + incline } else { PI/2.0 };
                (t, b.label.clone().unwrap_or_else(|| "N".into()), Color::Blue)
            }
            ForceEntry::Friction(b) => {
                ...
                (t, b.label.clone().unwrap_or_else(|| "f".into()), Color::Green)
            }
            ForceEntry::Applied(b) => {
                let deg = eval_num(&b.angle, vars).unwrap_or(0.0);
                (deg.to_radians(), b.label.clone().unwrap_or_else(|| "F".into()), Color::Purple)
            }
            ForceEntry::Tension(b) => (
                PI / 2.0, b.label.clone().unwrap_or_else(|| "T".into()), Color::Orange,
            ),
        };
        let tip = (center.0 + ARROW_LEN * theta.cos(), center.1 + ARROW_LEN * theta.sin());
        cetz::arrow(&mut s, center, tip, color);
        let lab_at = (center.0 + (ARROW_LEN + 0.25) * theta.cos(),
