...
pub fn render(spec: &FunctionGraph, vars: &VarMap) -> Result<String, DslError> {
    ...
    if xhi <= xlo || yhi <= ylo {
        return Err(DslError::Evaluation(
            "function_graph ranges must be ascending".into(),
        ));
    }
    ...
    let mut samples_by_label: std::collections::HashMap<String, Vec<(f64, f64)>> =
        Default::default();
    for fe in &spec.functions {
        let expr_str = vars
            .get(&fe.expr)
            .cloned()
            .unwrap_or_else(|| fe.expr.clone());
    ...
    for feat in &spec.features {
        ...
        let pts = match samples_by_label.get(&fb.of) {
            Some(p) => p,
            None => continue,
        };
        for (x, y) in find_features(pts, kind) {
            if !(xlo..=xhi).contains(&x) || !(ylo..=yhi).contains(&y) {
                continue;
            }
            ...
            if fb.label {
                ...
                let nudge = if matches!(kind, "min") {
                    -dy_off
                } else {
                    dy_off
                };
                let label_at = (x + dx_off, (y + nudge).clamp(ylo, yhi));
                let anchor = if nudge >= 0.0 {
                    "south-west"
                } else {
                    "north-west"
                };
